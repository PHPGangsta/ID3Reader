...
$result = mysql_query('DESCRIBE `'.mysql_real_escape_string(GETID3_DB_TABLE).'`');
...
if (!isset($ExistingTableFields['encoder_options'])) { // Added in 1.7.0b2
	...
	mysql_query('ALTER TABLE `'.mysql_real_escape_string(GETID3_DB_TABLE).'` ADD `encoder_options` VARCHAR(255) default '' NOT NULL AFTER `error`');
	mysql_query('OPTIMIZE TABLE `'.mysql_real_escape_string(GETID3_DB_TABLE).'`');
}
if (isset($ExistingTableFields['track']) && ($ExistingTableFields['track']['Type'] != 'varchar(7)')) { // Changed in 1.7.0b2
	...
	mysql_query('ALTER TABLE `'.mysql_real_escape_string(GETID3_DB_TABLE).'` CHANGE `track` `track` VARCHAR(7) default '' NOT NULL');
	mysql_query('OPTIMIZE TABLE `'.mysql_real_escape_string(GETID3_DB_TABLE).'`');
}
if (!isset($ExistingTableFields['track_volume'])) { // Added in 1.7.0b5
	...
	mysql_query('ALTER TABLE `'.mysql_real_escape_string(GETID3_DB_TABLE).'` ADD `track_volume` FLOAT NOT NULL AFTER `error`');
	mysql_query('OPTIMIZE TABLE `'.mysql_real_escape_string(GETID3_DB_TABLE).'`');
}
if (!isset($ExistingTableFields['remix'])) { // Added in 1.7.3b1
	echo '<b>adding field `encoder_options`, `alternate_name`, `parody`</b><br>';
	mysql_query('ALTER TABLE `'.mysql_real_escape_string(GETID3_DB_TABLE).'` ADD `remix` VARCHAR(255) default '' NOT NULL AFTER `title`');
	mysql_query('ALTER TABLE `'.mysql_real_escape_string(GETID3_DB_TABLE).'` ADD `alternate_name` VARCHAR(255) default '' NOT NULL AFTER `track`');
	mysql_query('ALTER TABLE `'.mysql_real_escape_string(GETID3_DB_TABLE).'` ADD `parody` VARCHAR(255) default '' NOT NULL AFTER `alternate_name`');
	mysql_query('OPTIMIZE TABLE `'.mysql_real_escape_string(GETID3_DB_TABLE).'`');
}
